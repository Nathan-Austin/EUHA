...
import { sendEmail, emailTemplates, emailBanner, emailEhcFooter } from '@/lib/email'
...
export async function lookupSauceByCodeForJudge(sauceCode: string) {
  const cookieStore = cookies();
  const supabase = createClient(cookieStore);
...
export async function sendVatEmail(supplierId: string) {
  ...
  // 3. Get all paid payments for this supplier in competition cycle
  // Competition cycle runs May-to-May (e.g., May 2025 - May 2026 for 2026 competition)
  // This accounts for entries opening in May and closing end of February
  const currentYear = COMPETITION_YEAR;
  const cycleStart = `${currentYear - 1}-05-01`;
  const cycleEnd = `${currentYear}-05-31`;
  ...
  if (!payments || payments.length === 0) {
    return { error: 'No paid entries found for this supplier in the current competition cycle.' };
  }
  ...
  const result = await generateShippingLabel({
    ...
    ...(customsRequired && {
      customs: {
        ...
        customsAmount: 5,
        customsCurrency: 'EUR',
        postalCharges: { value: 15, currency: 'EUR' },
        items: [
          {
...
function buildResultsHTML(brandName: string, sauces: SauceEmailData[]): string {
  ...
  return `${emailBanner}<div style="background:#08040e;padding:24px;font-family:Arial,sans-serif;max-width:600px;margin:0 auto;color:#fff"><h1 style="color:#ff4d00;margin-top:0">Your 2026 Results</h1><p>Dear ${brandName},</p>${intro}<p style="color:#888;font-size:13px">Scores are averaged across all judges on a scale of 1–10. We don't share individual judge details</p>${stickerNote}<hr style="border:none;border-top:1px solid #2a2a2a;margin:24px 0">${sauces.map(sauceBlock).join('')}<hr style="border:none;border-top:1px solid #2a2a2a;margin:24px 0"><p style="color:#888;font-size:13px">Results are published at <a href="https://heatawards.eu/results/2026" style="color:#ff4d00">heatawards.eu/results/2026</a>.</p><p style="color:#888;font-size:13px">Questions? Reply to this email or contact <a href="mailto:heataward@gmail.com" style="color:#ff4d00">heataward@gmail.com</a></p><p style="color:#555;font-size:12px;margin-top:24px">European Hot Sauce Awards 2026</p>${emailEhcFooter}</div>`
}
